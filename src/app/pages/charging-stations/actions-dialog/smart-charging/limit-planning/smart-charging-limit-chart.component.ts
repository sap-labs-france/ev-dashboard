...
export class SmartChargingLimitChartComponent implements OnInit {
  ...
  public refresh(limitPlanning: ConnectorSchedule[]) {
    this.limitPlanning = limitPlanning;
    if (this.limitPlanning && this.limitPlanning.length > 0) {
      this.createGraphData(this.limitPlanning);
    }
  }
...
/*      let tempDistance = Math.floor(connectorPlanning.slots.length / 200);
      ...
      // Line label
      const connectorLabel = this.translateService.instant('chargers.connector') + ' ' + this.connectorIdPipe.transform(index); 
      const axisId = "power";
