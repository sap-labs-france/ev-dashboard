...
//  @Input() currentChargeBoxID: string;
  ...
  constructor(
    ...
    // Check auth
    if (this.activatedRoute.snapshot.params['currentCharger.id'] &&
      !authorizationService.canUpdateChargingStation({'id': 'currentCharger.id'})) {
      // Not authorized
  ...
  ngOnInit() {
    ...
    if (this.currentCharger.id) {
      ...
      this.activatedRoute.params.subscribe((params: Params) => {
        this.currentCharger.id = params['id'];
        this.loadChargeBoxID();
      });
    }
    // Scroll up
  ...
  private _updateChargeBoxID(chargeBoxID) {
    ...
    this.centralServerService.updateChargingStationParams(this.currentCharger).subscribe(response => {
      ...
      if (response.status === Constants.REST_RESPONSE_SUCCESS) {
        // Ok
        this.messageService.showSuccessMessage(this.translateService.instant('chargers.change_config_success'));
        this.refresh();
      } else {
