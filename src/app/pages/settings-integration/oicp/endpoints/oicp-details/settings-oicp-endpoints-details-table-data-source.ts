...
export class SettingsOicpEndpointsDetailsTableDataSource extends TableDataSource<OicpEndpointDetail> {
    ...
    return new Observable((observer) => {
      ...
      if (this.oicpEndpoint) {
        ...
        oicpEndpointDetail = ({
          ...
          successNbr: this.oicpEndpoint.lastPatchJobResult ? this.oicpEndpoint.lastPatchJobResult.successNbr : 0,
          failureNbr: this.oicpEndpoint.lastPatchJobResult ? this.oicpEndpoint.lastPatchJobResult.failureNbr : 0,
          totalNbr: this.oicpEndpoint.lastPatchJobResult ? this.oicpEndpoint.lastPatchJobResult.totalNbr : 0,
          lastPatchJobOn: this.oicpEndpoint.lastPatchJobOn ? this.oicpEndpoint.lastPatchJobOn : null,
  ...
  public buildTableColumnDefs(): TableColumnDef[] {
    return [
      ...
      },
      {
        id: 'totalNbr',
        type: 'integer',
        name: this.oicpEndpoint ? (this.oicpEndpoint.role === 'CPO' ? 'oicpendpoints.total_charge_points' : 'oicpendpoints.total_tokens') : 'oicpendpoints.total',
        isAngularComponent: true,
  ...
  private enableDisableBackgroundJob(oicpendpoint: OicpEndpoint, enable: boolean) {
    ...
    this.dialogService.createAndShowYesNoDialog(
      (enable) ? this.translateService.instant('oicpendpoints.start_background_job_title')
