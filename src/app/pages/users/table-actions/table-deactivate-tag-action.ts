...
export class TableDeactivateTagAction extends TableDeactivateAction {
  ...
  private deactivateTag(tagID: string, dialogService: DialogService, translateService: TranslateService, messageService: MessageService,
    centralServerService: CentralServerService, spinnerService: SpinnerService, router: Router, refresh?: () => Observable<void>) {
    dialogService.createAndShowYesNoDialog(
      translateService.instant('tags.deactivate_title'),
      translateService.instant('tags.deactivate_confirm', { tagID }),
    ).subscribe((response) => {
      if (response === ButtonType.YES) {
        spinnerService.show();
        centralServerService.updateTagStatus(tagID, false).subscribe((actionResponse) => {
          spinnerService.hide();
          if (actionResponse.status === RestResponse.SUCCESS) {
            messageService.showSuccessMessage('tags.deactivate_success', { tagID });
            if (refresh) {
              refresh().subscribe();
            }
          } else {
