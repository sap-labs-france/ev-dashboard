        ...
        componentChanges: [
          {
            ...
            changes: [
              `Productive use of the new e-Mobility front-end`,
              `Charging Station > Details: Show connector detailed error`,
              `Charging chart ratio changes according screen resolution`,
              `Improve connector display`,
              `Fix no connector svg usage`,
              `Set the chart y-axis always to 0 as origin`,
              `User > Edit/Create: Saving now closes dialog`,
              `Esc/Enter handling in organizations, users, sessions and chargers pages`,
              `Scss clean-up: removed unused SCSS (classes, files, mixins, variables)`,
              `Remove transparent background to confirm pop-up`,
              `Edit action first in tenant table action row`,
              `Sidebar menu cannot be accessible if a pop-up is displayed`,
              `Fixes Charger connector types`,
              `Transactions > History: Handle details when Organization is not active`,
              `Refund: Set refundedAt as first column`,
              `Chargers: Handle registration of connectors in different orders`,
              `Use of mat-tabs components in all dialogs`,
              `Fixed CSS in toggle button in sidebar`,
              `Realigns right buttons correctly in mat-tabs`,
              `OCPI > Logs: Add OCPIPatchLocations to filter action in the drop down`                          ]
          },
          {
            name: 'Backend',
            changes: [
              `Fix Stop Transaction when Transaction is on ongoing and another user starts a new one`,
              `Remove docker configuration from assets`,
              `Remove the docker GCP code`,
              `Use the same body limit size for all body parsers in servers`,
              `Fix some class variable scope in servers initialization code`,
              `WSClient: Test properties on existing object`,
              `Add mandatory fields in status notification to be persisted in the charging station`,
              `package.json: update deps to latest`,
              `WSClient: Only reinstantiate callbacks when they actually do something`,
              `Issue Logging > Trace: fixed performance.clearMeasures() does not exist in NodeJs 10`,
              `Concur: Add quick expense V4 implementation`,
              `WSClient: Do not loose the event callbacks on ws on reconnection`,
              `WSClient: Fix a logical condition on the retry code path`,
              `Make the WSClient class a proxy ws class`,
              `EMail Notification: links in email jumps to the new Dashboard`,
              `Add battery levels checks in Unit Tests`,
              `Fixed bug with OCPP Clear Cache command`,
              `Small unit test fixes`,
              `Fixed delete of charger when only one connector is registered`,
              `Fixed issues in OCPP 1.5/1.6`,
              `Added ABB in unit tests`,
              `Improve a bit code readability`,
              `Remove verifyTenant tests`,
              `WSClient: add an auto reconnection logic to the WS client`,
              `WSServer: add a keep alive mechanism`,
              `Transaction refactoring`,
              `Organization: Disable REST API for Company/Site/Site Area if component is not active`,
              `Encapsulate all the web sockets client and server logic in their own classes`,
              `Add logs when an error occurred in convergent invoicing`,
              `Handle duplicate BootNotifications entries`,
              `OCPI - Logs - Use the same Action when logging`,
              `Moved OCPP 1.5 WSDLs into assets dir`,
              `Fix dates in Convergent Charging Pricing`,
              `Restore deleted methods removeChargingStationsFromSiteArea, addChargingStationsToSiteArea in ChargingStationStorage`,
              `Log when meter values with Sample Clock is rejected`,
              `Cleanup Postman files without passwords`,
              `Removed charger specifics unit test scripts`,
              `Ignored connector ID 0 for EBEE charger`                          ]
          }
        ]
      },
