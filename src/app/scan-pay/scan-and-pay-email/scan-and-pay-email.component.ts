...
@Component({
  ...
  templateUrl: 'scan-and-pay-email.component.html',
})

export class ScanAndPayEmailComponent implements OnInit, OnDestroy {
  ...
  public sendEmailVerificationScanAndPay(data: any) {
    this.reCaptchaV3Service.execute(this.siteKey, 'VerifScanPay', (token) => {
      ...
      // Yes: Update
      this.centralServerService.verifyScanPayEmail(data).subscribe({
        next: (response) => {
          this.spinnerService.hide();
