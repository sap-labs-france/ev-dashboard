...
import { AnalyticsSettings, AssetConnectionType, AssetSettings, AssetSettingsType, BillingSettings, BillingSettingsType, KeySettings, PricingSettings, PricingSettingsType, RefundSettings, RefundSettingsType, RoamingSettings, RoamingSettingsType, SmartChargingSettings, SmartChargingSettingsType } from '../types/Setting';
...
export class ComponentService {
  ...
  public getOicpSettings(): Observable<RoamingSettings> {
    return new Observable((observer) => {
      ...
      // Get the Pricing settings
      this.centralServerService.getSettings(TenantComponents.OICP).subscribe((settings) => {
        // Get the currency
        if (settings && settings.count > 0 && settings.result[0].content) {
          const config = settings.result[0].content;
          // Set
